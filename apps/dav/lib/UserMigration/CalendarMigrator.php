...
class CalendarMigrator {
	 ...
	 */
	public function getRequiredImportComponents(VCalendar $vCalendar, VObjectComponent $component) {
		$cleanComponent = $this->getCleanComponent($component);
		/** @var array<int, VTimeZone> $timezoneComponents */
		$timezoneComponents = $this->getTimezonesForComponent($vCalendar, $cleanComponent);
		return [
			...$timezoneComponents,
			$cleanComponent,
		];
